...
function groups_url($entity) {
    ...
    if (!elgg_is_logged_in()) {
        // Si el usuario no ha iniciado sesion, no deberia acceder a los grupos de discusion del grupo, sino a la pagina de informacion
        return "groups/info/{$entity->guid}/$title";
    } else {
        if ($entity->isMember(elgg_get_logged_in_user_entity())) {
            // Si la sesion ha sido iniciada Y el usuario es miembro de este grupo (comunidad o grupo de escritura), entonces debe ser redirigido a 
            // la pagina de los foros de discusion del grupo
            return "discussion/owner/{$entity->guid}";
        } else {
            // Si, a pesar de haber iniciado sesion, no es miembro de este grupo, lo logico es que el usuario observe la pagina de informacion del grupo
            return "groups/info/{$entity->guid}/$title";
        }
    }
}
