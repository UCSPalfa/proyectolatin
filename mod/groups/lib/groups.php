...
function groups_handle_edit_page($page, $guid = 0) {
    ...
    if ($page == 'add') {
        ...
        if ($group && $group->canEdit()) {
            ...
            elgg_push_breadcrumb($group->name, $group->getURL());
            elgg_push_breadcrumb($title);
            $content = elgg_view("groups/link", array('entity' => $group));
        } else {
            $content = elgg_echo('groups:noaccess');
        }
    }
...
function groups_handle_profile_page($guid) {
    ...
    if (group_gatekeeper(false)) {
        ...
        // Midification by Gonzalo
        // To show the groups that are linked to this group
        $sidebar .= elgg_view('groups/sidebar/links', array('entity' => $group));
...
function groups_handle_links_page($guid) {
    ...
    $title = elgg_echo('groups:link:title', array($group->name));

    elgg_push_breadcrumb($group->name, $group->getURL());
    elgg_push_breadcrumb(elgg_echo('groups:links'));
...
function groups_register_profile_buttons($group) {
    ...
    if ($group->canEdit()) {
        ...
        $url = elgg_get_site_url() . "groups/link/{$group->getGUID()}";
        $actions[$url] = 'groups:link';
    }
